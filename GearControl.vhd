...
ENTITY GearControl IS
	...
	PORT(char_to_write, plugboard_in: IN STD_LOGIC_VECTOR(4 DOWNTO 0);
		  ...
		  test_light: OUT STD_LOGIC;
		  gear_order: OUT gear_indices:= ("00", "01", "10");
		  gear_positions: BUFFER gear_positions:= ("00000", "00000", "00000");
...
BEGIN
	...
	BEGIN
		...
		ELSIF RISING_EDGE(clk) THEN
			test_light <= plugboard_in(0);
			confirm_prev <= confirm;
			mode_button_prev <= mode_button;
			-- Update mode
			IF mode_button_prev = '1' AND mode_button = '0' THEN
				CASE mode IS
					WHEN S_WRITE =>
						mode <= S_GEARS;
					WHEN S_GEARS =>
			...
			IF (mode = S_WRITE AND confirm = '0' AND confirm_prev = '1') THEN
				...
				END IF;
			-- Gear Select
			ELSIF mode = S_GEARS AND confirm_prev = '1' AND confirm = '0' AND TO_INTEGER(UNSIGNED(plugboard_in)) < 4 THEN
		...
		END IF;
	END PROCESS;
	display_chars <= char_to_write & plugboard_in & display_chars_internal(18) &
						  ...
						  display_chars_internal(2) & display_chars_internal(1) & display_chars_internal(0);
END;
