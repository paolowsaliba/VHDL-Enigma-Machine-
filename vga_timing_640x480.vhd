...
begin
  ...
  begin
    if rising_edge(clk50) then
      if rst = '1' then
        ...
        if to_integer(h_cnt) = H_TOTAL - 1 then
          h_cnt <= (others => '0');
          if to_integer(v_cnt) = V_TOTAL - 1 then
            v_cnt <= (others => '0');
          else
            v_cnt <= v_cnt + 1;
          end if;
  ...
  hsync <= '0' when (to_integer(h_cnt) >= (H_VISIBLE + H_FRONT) and
                     to_integer(h_cnt) <  (H_VISIBLE + H_FRONT + H_SYNC)) else '1';

  vsync <= '0' when (to_integer(v_cnt) >= (V_VISIBLE + V_FRONT) and
